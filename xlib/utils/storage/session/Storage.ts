  ...
  export class Storage extends storage.Storage implements IStorage {
        ...
        for (property in values) {
          if (values.hasOwnProperty(property)) {
            ...
            if (['null', 'undefined'].indexOf(xlib.typeOf(value)) !== -1) {
              if (window && window.sessionStorage &&
                xlib.typeOf(window.sessionStorage.removeItem) === 'function') {
                window.sessionStorage.removeItem(element);
              }
            } else {
              if (window && window.sessionStorage &&
                xlib.typeOf(window.sessionStorage.setItem) === 'function') {
                window.sessionStorage.setItem(element, JSON.stringify([this.getRevision(), temp === -1 ? -1 : Number(new Date()) + temp, value]));
              }
            }
          }
          this._changed = true;
    ...
    public getItems(keys: string[], callback?: (errors?: Error[], values?: any[]) => void): void {
      ...
      super.getItems(keys, () => {
        ...
        for (index = 0; index < length; index++) {
          ...
          if (window && window.sessionStorage &&
            xlib.typeOf(window.sessionStorage.getItem) === 'function') {
            try {
              value = JSON.parse(window.sessionStorage.getItem(element));
            } catch (e) {}
          }
          if (xlib.typeOf(value) === 'array' && value.length === 3) {
    ...
    public clear(callback?: (errors?: Error[]) => void): void {
      super.clear(() => {
        ...
        if (namespace === null) {
          if (window && window.sessionStorage &&
            xlib.typeOf(window.sessionStorage.clear) === 'function') {
