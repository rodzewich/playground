    ...
    function readFile(filename, callback) {
        ...
        if (content) {
            ...
            fs.readFile(resolve, function (error, result) {
                logger.debug("fs.readFile(" + resolve + ")", {
                    error  : error || null,
                    result : !!result
                });
                if (!error) {
                ...
                if (filename.substr(-3).toLowerCase() === ".js") {
                    ...
                    if (memoryFilesystem[pathname] && !memoryFilesystem[pathname].javascript) {
                        ...
                        memoryFilesystem[pathname].javascript = [
                            "define(" + JSON.stringify(pathname) + ", [" + references + "], function (module, exports, require, __extends, __decorate, __metadata, __param) {",
                    ...
                    if (memoryFilesystem[pathname] && !memoryFilesystem[pathname].sourcemap) {
                        ...
                        source.eachMapping(function (item) {
                            result.addMapping({
                                generated : {
                                    line   : item.generatedLine + 1, // todo: hardcode
    ...
    deferred([
        ...
        function (next) {
            if (useCache) {
                client.getItem(filename, function (error, response) {
                    logger.debug("client.getItem(" + String(filename) + ")", {
                        error  : error || null,
                        result : !!response
                    });
                    if (!error) {
            ...
            stat(filename, function (error, result) {
                if (!error) {
                    if (result.file) {
                        ...
                        logger.debug("stat(" + String(filename) + ")", {
                            mtime : mtime
                        });
                        next();
                    } else {
                        client.removeItem(filename, function (error) {
                            logger.debug("client.removeItem(" + String(filename) + ")", {
                                error : error || null
                            });
                            if (!error) {
                                done(null);
                        ...
                        function (next) {
                            if (references.length) {
                                client.getItems(references, function (error, result) {
                                    logger.debug("client.getItems([" + references.join(",") + "])", {
                                        error  : error || null,
                                        result : !!result
                                    });
        ...
        function (next) {
            client.lock(filename, function (error, result) {
                logger.debug("client.lock(" + filename + ")", {
                    error  : error || null,
                    result : !!result
                });
                if (!error) {
