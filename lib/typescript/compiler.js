...
function compile(options, callback) {
    ...
    function process(options, callback) {
        ...
        deferred([
            ...
            // connect to memory
            function (next) {
                client = memory({socket: "./temp/typescript/memory.sock"});
            ...
            function (next) {
                fs.stat(resolve + ".ts", function (err, stats) {
                    if (!err) {
                        if (stats.isFile()) {
                            mtime = new Date(Math.max(Number(stats.mtime), timeStart));
                            next();
                        } else {
            ...
            function (next) {
                deferred([
                    function (remove) {
                        client.getItem(filename, function (error, response) {
                            if (!error && response && response.date > Number(mtime)) {
            ...
            function (next) {
                readFile(filename, function (error, content) {
                    if (error) {
                        ...
                        currentDependencies = getFileReferences(filename, content);
                        next();
                    }
                });
            },
            ...
            function () {
                ...
                deferred([
                    ...
                    // lock process
                    function (next) {
                        client.lock(function (error, result) {
                            if (!error) {
                                unlock = result;
                                next();
                            } else {
