...
var MemoryClient = require("../memory/client/Client");
...
function Compiler(options) {
    ...
    var compress = typeOf(options.compress) ? !!options.compress : true;
    ...
    function getMemory(callback) {
        ...
        if (client) {
            ...
            temp = new MemoryClient({
                location: memorySocketLocation,
                namespace : "less"
            });
    ...
    self.compile = function (filename, callback) {
        ...
        var importDirs = importDirectories.slice(0);

        importDirs.unshift(path.join(sourcesDirectory, path.dirname(filename)));
