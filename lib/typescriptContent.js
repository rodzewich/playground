...
var TypescriptDiagnosticError = require("./typescriptDiagnosticError");
var TypescriptDiagnosticFileError = require("./typescriptDiagnosticFileError");
    ...
    function compile(filename, options) {
        ...
        ts.getPreEmitDiagnostics(program).concat(result.diagnostics).forEach(function (diagnostic) {
            ...
            if (diagnostic.file) {
                ...
                errors.push(new TypescriptDiagnosticFileError(message, filename, line, character));
            } else {
                message  = ts.flattenDiagnosticMessageText(diagnostic.messageText, '\n');
                code     = diagnostic.code;
                category = diagnostic.category;
                errors.push(new TypescriptDiagnosticError(message, code, category));
            }
        });
